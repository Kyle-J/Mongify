...
Resource.prototype.getItem = fucntion(callback)
{
     ...
     * Build the query and execute, callback with results
     */
    Resource.prototype.buildItemQuery(query).exec(callback)
 ...
 * @returns {*}
 */
Resource.prototype.buildItemQuery = function(query)
{
    /**
     * Set allowed fields if applicable
     */
    if(this.allowedFields.indexOf('*') !== -1) {

        if(this.state.fields != '*')
            query.select(this.state.fields.join(' '))
        else
            query.select(' -tenant_id')

    }else{

        var allowed = this.state.fields.filter(function(index) { return this.allowedFields.indexOf(index) != -1})
